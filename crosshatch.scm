...
(define (script-fu-crosshatch-make-etch image drawable name angle darkness)
  (let ((etch (script-fu-crosshatch-make-layer
	       image drawable name NORMAL-MODE)))
    (plug-in-noisify RUN-NONINTERACTIVE image etch 0 0.25 0.25 0.25 0.0)
...
(define (script-fu-crosshatch image drawable darkness
...
; The crosshatching is two layers 90 degrees apart
  (script-fu-crosshatch-make-etch image drawable "Etch" 135 darkness)
  (script-fu-crosshatch-make-etch image drawable "Cross Etch" 45 darkness)
; Clean up
